...
describe("JunToken", function () {
    ...
    describe("test each functions", function () {
        it('send to dev', async function() {
            ...
            expect(await jtb.checkLicensorLength()).to.equal(1);
        ...
        it('transfer to dev', async function () {
            ...
            expect(await jtb.checkLicensorLength()).to.equal(2);
    ...
    describe("test add user into white list", function () {
        it('call the function get white list', async function () {
            ...
            expect(await jtb.checkLicensorLength()).to.equal(1);
            let licensorAddress = await jtb.checkLicensor();
            expect(licensorAddress[0]).to.equal(owner.address);
            await expect(jtb.addWhiteList(dev.address)).to.emit(jtb, "AddWhiteList");
            await expect(jtb.connect(dev).addWhiteList(third.address)).to.reverted;
            expect(await jtb.checkLicensorLength()).to.equal(1);
            let newLicensorAddress = await jtb.checkLicensor();
            expect(newLicensorAddress[0]).to.equal(owner.address);
            await expect(jtb.translateToLicensor(dev.address)).to.emit(jtb, "TranslateToLicensor");
            expect(await jtb.checkLicensorLength()).to.equal(2);
            let licensorSecondAddress = await jtb.checkLicensor();
            expect(licensorSecondAddress[1]).to.equal(dev.address);
            await expect(jtb.connect(dev).addWhiteList(third.address)).to.emit(jtb, "AddWhiteList");
            let toBeThird = await jtb.checkGrantee(dev.address);
            expect(toBeThird[0]).to.equal(third.address);
        ...
        it('call the function remove white list', async function () {
            ...
            let granteeUserNameArray = await jtb.checkGrantee(dev.address);
            expect(granteeUserNameArray[0]).to.equal(ZEROADDRESS);
        ...
        it('call the function check grantee', async function () {
            const { owner, dev, jtb } = await loadFixture(getJunToken);

            await expect(jtb.addWhiteList(dev.address)).to.emit(jtb, "AddWhiteList");
            let granteeName = await jtb.checkGrantee(owner.address);
            expect(granteeName[0]).to.equal(dev.address);
        });
    });
});